...
const masterItemImageMap = {
  '/party': '/party_converted.avif',
  '/stock': '/stock item_converted.avif'
};
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#0f172a] to-[#020617] px-4 py-6">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl items-center justify-center">
        <div className="relative flex w-full max-w-[23rem] flex-col overflow-hidden rounded-[30px] border border-white/20 bg-gradient-to-br from-white/95 via-white/90 to-white/80 shadow-[0_32px_80px_rgba(0,0,0,0.5),0_0_60px_rgba(59,130,246,0.15),inset_0_1px_0_rgba(255,255,255,0.8)] backdrop-blur-sm">
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.12),transparent_40%),radial-gradient(circle_at_80%_20%,rgba(6,182,212,0.1),transparent_30%),radial-gradient(circle_at_bottom_right,rgba(14,165,233,0.08),transparent_30%)]" />
          ...
          <div className="sidebar-scrollbar relative z-10 flex-1 overflow-y-auto pb-8">
            <nav className="flex flex-col">
              {items.map((item, index) => {
                ...
                const itemImageSrc = masterItemImageMap[item.path];
                ...
                return (
                  <Link
                    ...
                    className={`group relative flex items-center gap-3 border-b border-slate-200/90 px-5 py-2.5 text-[12px] transition-colors duration-200 last:border-b-0 ${
                      isActive
                        ? 'bg-[linear-gradient(90deg,rgba(219,234,254,0.96),rgba(239,246,255,0.94))] text-slate-800'
                        : 'text-slate-700 hover:bg-sky-50/90'
                    }`}
                  >
                    {isActive && <div className="absolute inset-y-0 left-0 w-1 bg-sky-500" />}

                    <div className="flex h-10 w-10 shrink-0 items-center justify-center">
                      {itemImageSrc ? (
                        <img src={itemImageSrc} alt={item.name} className="h-10 w-10 object-contain" />
                      ) : (
                        <ItemIcon />
                      )}
                    </div>
